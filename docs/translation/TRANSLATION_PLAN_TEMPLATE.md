# 📋 논문 번역 진행 계획 (Translation Plan): [논문 ID]

## 🎯 목표
`orig.html`의 내용을 누락 없이 동일한 수준의 가독성으로 `ko.html`에 반영한다. (단순 요약이 아닌 전체 본문 번역)

## 🔄 워크플로우 (Workflow)
1. **분석 (Analyze)**: `orig.html`에서 번역할 섹션의 전체 텍스트와 구조(Table, Figure, List) 추출.
2. **작업 (Translate)**: `docs/translation/words.md`를 참조하여 섹션별 상세 번역 수행.
3. **통합 (Integrate)**: 번역된 내용을 `ko.html`의 해당 위치에 삽입 및 스타일 유지.
4. **검증 (Verify)**: 원문과 대조하여 누락 여부 및 용어 통일성 체크.

## 📝 To-do Checklist

### [Phase 1] 기초 섹션 보강 (Executive Summary & Intro)
- [ ] 용어집(`docs/translation/words.md`) 업데이트 및 보강
- [ ] 제목 및 저자 정보 확인
- [ ] 초록(Abstract) 및 서론(Introduction) 번역 반영

### [Phase 2] 본문 정밀 번역 (Section 1-3)
- [ ] 주요 섹션 상세 번역
- [ ] 수식, 리스트, 강조 텍스트 스타일 적용
- [ ] 그림(Figure) 및 표(Table) 위치 확보

### [Phase 3] 본문 정밀 번역 (Section 4-End)
- [ ] 남은 본문 섹션 전체 번역
- [ ] 결론(Conclusion) 및 감사의 글(Acknowledgment)

### [Phase 4] 시각 자료 및 데이터 통합
- [ ] 이미지(Figures) 캡션 번역 및 삽입
- [ ] 표(Tables) 데이터 한글화 및 레이아웃 조정
- [ ] 참고문헌(References) 구조화

### [Phase 5] 최종 검토 및 폴리싱
- [ ] 전체 레이아웃 및 반응형 확인
- [ ] 스크롤 동기화 동작 테스트
- [ ] 오탈자 및 용어 일관성 최종 점검

---
*이 계획은 [논문 ID]의 번역 품질을 높이기 위해 작성되었습니다.*
